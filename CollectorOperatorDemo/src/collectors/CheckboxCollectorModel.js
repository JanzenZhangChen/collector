...
const CheckboxReact = ({list = [], datas = [], onChange = () => {}, error, label}) => {
    return (
        <div>
            <div>{label}</div>
            {list.map((item) => {
                return (
                    <span key={item.value}>
                        {item.label}
                        <input onChange={(e) => {onChange(item)}} type="checkbox" checked={datas.includes(item.value)}/>
                    </span>
                )
            })}
    ...
    _services: {
        onChange: (item) => (dispatch, getState$, collector) => {
            ...
            if (datas.includes(item.value)) {
                ...
                newDatas = datas.concat([item.value])
            }
